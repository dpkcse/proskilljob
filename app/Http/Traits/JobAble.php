...
trait JobAble
{
    ...
    {
        if (auth()->user()) {
            $query = Job::with('company.user', 'category', 'profession:id', 'job_type:id')
                ->withCount([
                    'bookmarkJobs',
                    'appliedJobs',
        ...
        if ($request->has('profession') && $request->profession != null) {
            // Find the profession id based on the translated 'name' for the current locale
            $profession_id = Profession::whereTranslation('name', $request->profession, app()->getLocale())->value('id');
        ...
        if ($request->has('location') && $request->location != null) {
            ...
            // Replace special characters and split the location by commas or plus signs
            $address_array = preg_split('/[,+\s]+/', $address);
            $address_array = array_map('trim', $address_array); // Trim spaces
            $address_array = array_filter($address_array); // Remove empty elements

            // Prepare the query to match any part of the address
            $query->where(function ($query) use ($address_array) {
                foreach ($address_array as $part) {
                    $query->orWhere('country', 'LIKE', '%'.$part.'%')
                        ->orWhere('address', 'LIKE', '%'.$part.'%')
                        ->orWhere('district', 'LIKE', '%'.$part.'%')
                        ->orWhere('place', 'LIKE', '%'.$part.'%');
                }
            });
        }
    ...
    public function call($fname, $args)
    {
        ...
        $url = 'https://public.api.careerjet.net/'.$fname.'?locale_code='.$locale;
    ...
    public function toggleApplyJob(Request $request)
    {
        ...
        if ($job->company->user && authUser()->recent_activities_alert) {
            auth('user')
                ->user()
