...
class JobsImport implements ToModel, WithBatchInserts, WithChunkReading, WithStartRow
{
    ...
    public function model(array $row)
    {
        ...
        $job_start = isset($row[20]) && ! empty($row[20]) ? $this->dateFormat($row[20]) : null;
        $job_end = isset($row[21]) && ! empty($row[21]) ? $this->dateFormat($row[21]) : null;
    ...
    {
        if (isset($data) && ! empty($data)) {
            $job_type = JobType::whereHas('translations', function ($query) use ($data) {
