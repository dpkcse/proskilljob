...
class TranslationController extends Controller
{
    ...
    public function langView($code)
    {
        try {
            ...
            $originalEnglish = json_decode(file_get_contents(base_path('resources/lang/en.json')), true);
            ...
            if (! empty($keyword)) {
                $translations = array_filter($translations, function ($key, $value) use ($keyword) {
                    // You can customize the condition based on your requirements
                    return stripos($value, $keyword) !== false || stripos($key, $keyword) !== false;
            ...
            return view('language::lang_view', compact('language', 'translations', 'keyword', 'originalEnglish'));
        } catch (\Exception $e) {
            flashError('An error occurred: '.$e->getMessage());
