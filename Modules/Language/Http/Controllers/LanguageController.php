...
class LanguageController extends Controller
{
    ...
    public function syncLanguage(Language $language)
    {
        try {
            ...
            // We remove the keys that are not in the English language
            $translations = array_intersect_key($translations, $en_translations);
    ...
    private function syncLanguageJson($code)
    {
        try {
            ...
            // English Language Translations
            $en_translations = json_decode(file_get_contents(base_path('resources/lang/en.json')), true);

            // We remove the keys that are not in the English language
            $mergedJson = array_intersect_key($mergedJson, $en_translations);
