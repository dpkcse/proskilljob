                        ...
                        <div class="row">
                            <table class="table table-striped table-bordered mt-0 pt-0" id="tranlation-table"
                                ...
                                <thead>
                                    <tr>
                                        <th>#</th>
                                        <th width="20%">{{ __('key') }}</th>
                                        <th width="40%">{{ __('english_text') }}</th>
                                        <th width="40%">
                                ...
                                <tbody>
                                    @php
                                    $lastPageTotalData = request()->input('page') == null || request()->input('page') ==
                                    1 ? 0 : request()->input('page') * 100;
                                    @endphp
                                    @foreach ($translations as $key => $value)
                                    <tr>
                                        <td>{{ $lastPageTotalData + $loop->iteration }}</td>
                                        <td class="key" style="word-break: break-all;">{{ $key }}
                                        <td>{{ $originalEnglish[$key] ?? '' }}
                                        </td>
                                        <td>
                                            <span class="d-flex">
                                                <input type="text" class="form-control value  w-100" name="{{ $key }}"
                                                    value="{{ $value }}">
                                                <button type="button"
