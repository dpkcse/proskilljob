        ...
        <div class="ll-invoice-in">
            <div class="ll-invoice-head">
                <div class="ll-invoice-logo">
                    <img src="{{ $setting->light_logo_url }}" alt="logo" style="height: 80px">
                </div>
            ...
            <div class="ll-invoice-info">
                ...
                <div class="ll-invoice-info_right">
                    <div><span class="span-1">Invoice No:</span> <b>#{{ $transaction->id }}</b></div>
            ...
            <div class="ll-invoice-pay-to">
                ...
                <div class="right-side">
                    ...
                    <p style="margin-bottom: 15px;">
                        ...
                        @if($transaction->user->contactInfo && $transaction->user->contactInfo->address)
                            {{ $transaction->user->contactInfo->address }} <br>
                        @endif
                        @if($transaction->user->contactInfo && $transaction->user->contactInfo->phone)
                            {{ $transaction->user->contactInfo->phone }}
                        @endif
                    </p>
                </div>
            </div>
            <div class="ll-invoice-table-wrap">
                <div class="ll-invoice-main-table">
                    <table>
                        ...
                        <tbody>
                            <tr>
                                <td class="w-25">{{ $transaction->plan->name }}</td>
                                <td class="w-32">
                                    <span>Job Apply Limit: {{ $transaction->plan->job_apply_limit }}</span> <br>
                                    @if($transaction->plan->featured_profile)
                                        <span>Featured Profile: Yes</span> <br>
                                    @endif
                                    @if($transaction->plan->profile_views)
                                        <span>Profile Views: Yes</span>
                                    @endif
                                </td>
                                <td class="w-16">{{ currencyConversion($transaction->amount, 'USD') }} {{ currentCurrencyCode() }}</td>
                                <td class="w-8">1</td>
                                <td class="w-8 text-right">{{ currencyConversion($transaction->amount, 'USD') }} {{ currentCurrencyCode() }}</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
                <div class="ll-invoice-footer-table">
                    <div class="left-side">
                        ...
                        <p>
                            Payment Method - {{ ucfirst($transaction->payment_method) }} <br>
                            Payment Status: {{ ucfirst($transaction->payment_status) }} Paid {{ __('paid') }} <br>
                        ...
                        <table>
                            <tbody>
                                <tr style="background: #f5f6fa;">
                                    <td style="color: #111; font-weight: 700;border-top: 1px solid #dbdfea;">Subtotal</td>
                                    <td class="text-right" style="color: #111; font-weight: 700;border-top: 1px solid #dbdfea;">
                                        {{ currencyConversion($transaction->amount, 'USD') }} {{ currentCurrencyCode() }}
                                    </td>
                                </tr>
            ...
            <div style="position: absolute; bottom: 30px; left:50px; right: 50px; text-align: center;">
                ...
                <a href="{{ route('website.termsCondition') }}" target="_blank" rel="noopener noreferrer">View Terms & Condition</a>
            </div>
        </div>
