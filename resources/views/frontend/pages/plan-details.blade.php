    ...
    <section class="section benefits bgcolor--gray-10 mt-5 pt-5">
        <div class="container">
            <div class="row mt-4">
                <div class="col-md-6">
                    <div class="card p-3">
                        <h5 class="mb-2">{{ __('Have a coupon?') }}</h5>
            ...
            <div id="discount-info" class="row mt-2" style="display:none;">
                <div class="col-12">
            ...
            <div class="row py-5">
                ...
                @if (config('paypal.active') ||
                    ...
                    @endif
                    @if (config('templatecookie.Iyzipay_live_mode') && config('templatecookie.Iyzipay_api_key') && config('templatecookie.Iyzipay_api_secret') && config('templatecookie.Iyzipay_base_url'))
                    <div class="col-4 my-2">
                        <div class="card jobcardStyle1">
                            <div class="card-body">
                                <div class="rt-single-icon-box">
                                ...
                                <div class="post-info d-flex">
                                    <div class="flex-grow-1">
                                        <button id="iyzipay_btn" type="button" class="btn btn-primary2-50 d-block">
                                            {{ __('pay_now') }}
                                        </button>
                                    </div>
                                </div>
        ...
        <form action="{{ route('iyzipay.post') }}" method="POST" class="d-none" id="iyzipay-form">
            @csrf
        </form>
            ...
            $.ajax({
                ...
                success: function(response) {
                    if(response.success) {
                        ...
                        // Optionally update total price on the page
                        $('.text-info').text("{{ __('total_amount_to_pay') }}: " + response.new_total);
                    } else {
                        $('#discount-info').hide();
                ...
                error: function(xhr) {
                    $('#discount-info').hide();
                    $('#coupon-message').html('<span class="text-danger">An error occurred. Please try again.</span>');
                }
            });
        });
    </script>
