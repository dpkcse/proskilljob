    ...
    <div class="row justify-content-center align-items-center pt-5 mt-5">
        <div class="col-lg-4">
            ...
            <div class="auth-box2 mx-auto">
                <form method="POST" action="{{ route('verification.resend.mail') }}" class="rt-form">
                    @csrf
                    @if(isset($email))
                    ...
                    @if(isset($email))
                        <div class="alert alert-info text-center rt-mb-20">
                            <small>{{ __('verification_email_sent_to') }} <strong>{{ $email }}</strong></small>
                        </div>
                    @endif

                    <span class="d-block body-font-3 text-center text-gray-600 rt-mb-32">
                        {{ __('we_have_sent_an_verification_to_your_email_address_click_the_to_verify_your_email_address_and_activate_your_account') }}
                    </span>
                ...
                <div class="text-center rt-mt-20">
                    <a href="{{ route('register') }}" class="text-decoration-none">
                        {{ __('back_to_registration') }}
                    </a>
                </div>
            </div>
        </div>
