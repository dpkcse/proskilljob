...
<script>
    ...
    function syncMapLocationFields(form) {
        if (!form || typeof form.get !== 'function') {
            return;
        }

        $('[data-map-location-field]').each(function() {
            const field = $(this).data('map-location-field');
            const value = field === 'location' ? '1' : form.get(field);
