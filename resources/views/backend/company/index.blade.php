            ...
            <div class="col-md-12">
                <div class="card">
                    ...
                    <div class="card-body table-responsive p-0">
                        ...
                        <form id="bulk-delete-form" action="{{ route('company.bulk.delete') }}" method="POST">
                            @csrf
                            <table class="ll-table table table-hover text-nowrap">
                                ...
                                <tbody>
                                    @forelse ($companies as $company)
                                        <tr>
                                            <td>
                                                <input type="checkbox" name="ids[]" value="{{ $company->id }}" class="company-checkbox">
                                            </td>
                                            <td>
                                                ...
                                                <td>
                                                    ...
                                                    @if (userCan('company.delete'))
                                                        <form action="{{ route('company.destroy', $company->id) }}"
                                                            method="POST" class="d-inline individual-delete-form">
                                                            @method('DELETE')
                                                            @csrf
                                                            <button
                                                                type="button"
                                                                onclick="if(confirm('{{ __('are_you_sure_you_want_to_delete_this_item') }}')) { this.form.submit(); }"
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            function toggleIndividualDeleteButtons() {
                var hasSelected = $('.company-checkbox:checked').length > 0;
            ...
            $('#bulk-delete').click(function(e) {
                e.preventDefault();
                var selectedCompanies = $('.company-checkbox:checked');

                if (selectedCompanies.length === 0) {
                    toastr.error('{{ __("please_select_at_least_one_company") }}');
                    return;
                }

                if (confirm('{{ __("are_you_sure_you_want_to_delete_selected_companies") }}')) {
                    var companyIds = [];
                    selectedCompanies.each(function() {
                        companyIds.push($(this).val());
                    });

                    // AJAX request to delete selected companies
                    $.ajax({
                        url: '{{ route('company.bulk.delete') }}',
                        type: 'POST',
                        data: {
                            ids: companyIds,
                            _token: '{{ csrf_token() }}'
                        ...
                        success: function(response) {
                            toastr.success('{{ __("selected_companies_deleted_successfully") }}');
