            ...
            <div class="ll-card">
                ...
                <div class="desc-skills-wrapper">
                    <div class="desc-wrap">
                        ...
                        <div class="row">
                            <div class="col-md-4 mb-2"><strong>{{ __('father_name') }}:</strong>
                                {{ $candidate->father_name ?: __('not_available') }}</div>
                            <div class="col-md-4 mb-2"><strong>{{ __('mother_name') }}:</strong>
                                {{ $candidate->mother_name ?: __('not_available') }}</div>
                        </div>
                    </div>
                    <div class="desc-wrap">
                        <div class="text-bold">
                ...
                <div class="desc-skills-wrapper mt-3">
                    <div class="desc-wrap">
                        <div class="text-bold mb-2">{{ __('education') }}</div>
                        @if ($candidate->educations->count())
                            <ul class="mb-0 pl-3">
                                @foreach ($candidate->educations as $education)
                                    <li>
                                        {{ $education->degree_name ?: ($education->exam_name ?: __('not_available')) }} -
                                        {{ $education->institute_name ?: __('not_available') }}
                                        ({{ $education->passing_year ?? __('not_available') }})
                                    </li>
                                @endforeach
                            </ul>
                        @else
                            <p>{{ __('not_available') }}</p>
                        @endif
                    </div>
                    <div class="skills-wrap">
                        <div class="text-bold mb-2">{{ __('experience') }}</div>
                        @if ($candidate->experiences->count())
                            <ul class="mb-0 pl-3">
                                @foreach ($candidate->experiences as $experience)
                                    <li>
                                        {{ $experience->designation ?: __('not_available') }} -
                                        {{ $experience->company ?: __('not_available') }}
                                        ({{ $experience->formatted_start ?? '-' }} - {{ $experience->currently_working ? __('present') : ($experience->formatted_end ?? __('present')) }})
                                    </li>
                                @endforeach
                            </ul>
                        @else
                ...
                <div class="desc-skills-wrapper mt-3">
                    <div class="desc-wrap w-100">
                        <div class="text-bold mb-2">{{ __('reference') }}</div>
                        @if ($candidate->professionalReferences->count())
                            <div class="table-responsive">
                                <table class="table table-bordered">
                                    ...
                                    <tbody>
                                        @foreach ($candidate->professionalReferences as $reference)
                                            <tr>
                                                <td>{{ $reference->name ?? __('not_available') }}</td>
                                                <td>{{ $reference->designation ?? __('not_available') }}</td>
                                                <td>{{ $reference->mobile ?? ($reference->phone_off ?? __('not_available')) }}</td>
                                                <td>{{ $reference->email ?? __('not_available') }}</td>
                                            </tr>
                                        @endforeach
                                    </tbody>
                                </table>
