...
@section('title')
    {{ __('application_tracking_system') }}
@endsection
@section('content')
    @php
        $userr = auth()->user();
            ...
            <div class="card">
                <div class="card-header">
                    <div class="d-flex flex-wrap justify-content-between">
                        <h3 class="card-title line-height-36">{{ __('application_tracking_system') }}</h3>
                ...
                {{-- Filter  --}}
                <form id="formSubmit" action="{{ route('applied.jobs') }}" method="GET" onchange="this.submit();">
                    <div class="card-body border-bottom row">
                        ...
                        <div class="col-xl-3 col-lg-4 col-md-6 col-12">
                            ...
                            <select name="application_status" class="form-control select2bs4 w-100-p">
                                <option value="">{{ __('all') }}</option>
                                <option value="{{ __('shortlisted') }}" {{ request('application_status') == __('shortlisted') ? 'selected' : '' }}>
                                    {{ __('shortlisted') }}
                                </option>
                                <option value="{{ __('interview') }}" {{ request('application_status') == __('interview') ? 'selected' : '' }}>
                                    {{ __('interview') }}
                                </option>
                                <option value="{{ __('rejected') }}" {{ request('application_status') == __('rejected') ? 'selected' : '' }}>
                                    {{ __('rejected') }}
                                </option>
                            </select>
                        </div>
                        <div class="col-xl-2 col-lg-4 col-md-6 col-12 d-flex align-items-end">
                            @if (request('keyword') || request('application_status'))
                                <a href="{{ route('applied.jobs') }}" class="btn bg-danger"><i
                ...
                <div class="card-body border-bottom">
                    <div class="row">
                        <div class="col-md-3 col-6 mb-2">
                            <span class="badge badge-primary">{{ __('total') }}: {{ $statusSummary['total'] }}</span>
                        </div>
                        <div class="col-md-3 col-6 mb-2">
                            <span class="badge badge-info">{{ __('shortlisted') }}: {{ $statusSummary['shortlisted'] }}</span>
                        </div>
                        <div class="col-md-3 col-6 mb-2">
                            <span class="badge badge-success">{{ __('interviewed') }}: {{ $statusSummary['interviewed'] }}</span>
                        </div>
                        <div class="col-md-3 col-6 mb-2">
                            <span class="badge badge-danger">{{ __('rejected') }}: {{ $statusSummary['rejected'] }}</span>
                        </div>
                    </div>
                </div>
                                    ...
                                    @if ($applied_jobs->count() > 0)
                                        @foreach ($applied_jobs as $job)
                                            <tr>
                                                    ...
                                                    <td tabindex="0">
                                                        <span class="badge
                                                            @if (strtolower(optional($job->applicationGroup)->name) == strtolower(__('shortlisted'))) badge-info
                                                    ...
                                                    <td tabindex="0">
                                                        <p>{{ Str::limit($job->cover_letter, 70) }}</p>
                                                    </td>
                                                <td>
