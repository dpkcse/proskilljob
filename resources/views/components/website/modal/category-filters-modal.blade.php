...
<div class="modal fade" id="filtersModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div
        ...
        <div class="modal-content tw-rounded-none tw-relative tw-min-h-screen tw-max-h-screen">
                ...
                <div class="tw-p-5">
                    ...
                    <div class="tw-flex tw-justify-between tw-items-center tw-mb-4">
                        <p class="tw-text-sm tw-text-[#767F8C] tw-mb-0">Min:
                            @if($currentCurrency->symbol_position == 'left')
                                {{ $currentCurrency->symbol }}<span>0</span>
                            @else
                                <span>0</span>{{ $currentCurrency->symbol }}
                            @endif
                        </p>
                        <p class="tw-text-sm tw-text-[#767F8C] tw-mb-0">Max:
                            @if($currentCurrency->symbol_position == 'left')
                                {{ $currentCurrency->symbol }}<span>{{ round($maxSalary, 0) }}</span>
                            @else
                                <span>{{ round($maxSalary, 0) }}</span>{{ $currentCurrency->symbol }}
                            @endif
                        </p>
                    </div>
                    <div class="tw-flex tw-gap-2 tw-items-center tw-py-2">
                        <input {{ request('price_min') == 10 && request('price_max') == 100 ? 'checked' : '' }}
                            ...
                            name="salleryRange">
                        <label for="10" class="tw-text-sm tw-text-[#18191C] tw-mt-[2px]">
                            @if($currentCurrency->symbol_position == 'left')
                                {{ $currentCurrency->symbol }}10 - {{ $currentCurrency->symbol }}100
                            @else
                                10{{ $currentCurrency->symbol }} - 100{{ $currentCurrency->symbol }}
                            @endif
                        </label>
                    </div>
                    <div class="tw-flex tw-gap-2 tw-items-center tw-py-2">
                        <input {{ request('price_min') == 100 && request('price_max') == 1000 ? 'checked' : '' }}
                            ...
                            name="salleryRange">
                        <label for="100" class="tw-text-sm tw-text-[#18191C] tw-mt-[2px]">
                            @if($currentCurrency->symbol_position == 'left')
                                {{ $currentCurrency->symbol }}100 - {{ $currentCurrency->symbol }}1,000
                            @else
                                100{{ $currentCurrency->symbol }} - 1,000{{ $currentCurrency->symbol }}
                            @endif
                        </label>
                    </div>
                    <div class="tw-flex tw-gap-2 tw-items-center tw-py-2">
                        <input {{ request('price_min') == 1000 && request('price_max') == 10000 ? 'checked' : '' }}
                            ...
                            name="salleryRange">
                        <label for="1000" class="tw-text-sm tw-text-[#18191C] tw-mt-[2px]">
                            @if($currentCurrency->symbol_position == 'left')
                                {{ $currentCurrency->symbol }}1,000 - {{ $currentCurrency->symbol }}10,000
                            @else
                                1,000{{ $currentCurrency->symbol }} - 10,000{{ $currentCurrency->symbol }}
                            @endif
                        </label>
                    </div>
                    <div class="tw-flex tw-gap-2 tw-items-center tw-py-2">
                        <input {{ request('price_min') == 10000 && request('price_max') == 100000 ? 'checked' : '' }}
                            ...
                            name="salleryRange">
                        <label for="10000" class="tw-text-sm tw-text-[#18191C] tw-mt-[2px]">
                            @if($currentCurrency->symbol_position == 'left')
                                {{ $currentCurrency->symbol }}10,000 - {{ $currentCurrency->symbol }}100,000
                            @else
                                10,000{{ $currentCurrency->symbol }} - 100,000{{ $currentCurrency->symbol }}
                            @endif
                        </label>
                    </div>
                    <div class="tw-flex tw-gap-2 tw-items-center tw-py-2">
                        <input {{ request('price_min') >= 1000000 && !request('price_max') ? 'checked' : '' }}
                            ...
                            name="salleryRange">
                        <label for="100000Up" class="tw-text-sm tw-text-[#18191C] tw-mt-[2px]">
                            @if($currentCurrency->symbol_position == 'left')
                                {{ $currentCurrency->symbol }}100,000+
                            @else
                                100,000{{ $currentCurrency->symbol }}+
                            @endif
                        </label>
                    </div>
                </div>
            ...
            <div
                class="tw-absolute tw-bottom-0 tw-left-0 tw-right-0 tw-p-5 tw-bg-white tw-z-50 tw-flex tw-justify-between tw-items-center tw-mt-3">
...
@push('frontend_scripts')
    <script>
        ...
        function categoryClose() {
            $('input[type="radio"].category-radio').prop('checked', false);
            $('#allcategory').prop('checked', true).trigger('change');
            $('#job_search_form').submit();
        }
